      ...
      <div class="p-5">
        <a href="#">
          <h5 class="mb-2 text-2xl font-bold tracking-tight text-center text-blue-900 dark:text-white">
            ควบคุมการเปิดปิดไม้กั้นทางออก</h5>
        </a>
...
export default {
  setup() {
    ...
    const fetchData = async () => {
      const barrier_in_on = dbRef(realtime, 'barrier_out_on'); // กำหนด path ไปยัง field ที่ต้องการ
      const barrier_in_off = dbRef(realtime, 'barrier_out_off'); // กำหนด path ไปยัง field ที่ต้องการ
      const barrier_in_auto = dbRef(realtime, 'barrier_out_auto'); // กำหนด path ไปยัง field ที่ต้องการ
      const out_distance1 = dbRef(realtime, 'out_distance1');
      const out_distance2 = dbRef(realtime, 'out_distance2');
      const out_distance3 = dbRef(realtime, 'out_distance3');
      const out_distance4 = dbRef(realtime, 'out_distance4');
    ...
    const toggleOn = async () => {
      ...
      const updates = {};
      updates['barrier_out_on'] = true;
      updates['barrier_out_off'] = false;
      updates['barrier_out_auto'] = false;
    ...
    const toggleOff = async () => {
      const updates = {};
      updates['barrier_out_on'] = false;
      updates['barrier_out_off'] = true;
      updates['barrier_out_auto'] = false;
      await update(dbRef(realtime), updates);
      fetchData(); // เรียก fetchData เพื่อรีเฟรชค่าใน fieldValue
      isOn.value = false;
